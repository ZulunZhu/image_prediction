...
def process_dataset(root_dir, patch_dir, bbox, base_filename='b01_16r4alks'):

    # Create a directory "image_data" in the current folder to store outputs
    save_dir = os.path.join(patch_dir, "image_data")
    ...
    subdirs = [d for d in os.listdir(root_dir) if d.startswith("cor_") and os.path.isdir(os.path.join(root_dir, d))]
...
if __name__ == '__main__':
    ...
    # Define inputs
    root_directory = "/aria/zulun_sharing/TGNN_sharing_20250214/merged/cors" # Root directory containing the "cor_YYYYMMDD_YYYYMMDD" folders.
    patch_size = 224  # Size of each patch
    patch_overlap = 10  # Overlap between patches
    sys.setrecursionlimit(100000)  # Increase recursion limit to avoid crashes

    # Get list of patches
    sub_directory = [d for d in os.listdir(root_directory) if d.startswith("cor_") and os.path.isdir(os.path.join(root_directory, d))]
    width, length = sizeFromXml(os.path.join(root_directory, sub_directory[0], "b01_16r4alks.cor"))   # Get size of full image
    patchList = computePatches(width, length, patch_size, patch_overlap)   # Get list of bounding boxes for each patch

    # To-do: parallelise the processing of each patch, each patch can be done independently
    for patch_id, patch_bbox in enumerate(patchList):

        # Create sub-directory for each patch
        patch_dir = os.path.join(os.getcwd(),'patch_' + f"{patch_id:03d}")
        if not os.path.exists(patch_dir):
            os.makedirs(patch_dir)

        # Process dataset for each patch
        node_features, edge_features, node_mapping, edge_node_pairs = process_dataset(root_directory, patch_dir, patch_bbox, base_filename='b01_16r4alks')
